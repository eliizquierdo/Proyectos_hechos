...
/**
 * Ventana de diálogo que muestra una lista de programación (series y películas).
 */
public class DlgListProgramming extends JDialog {
     ...
     */
    public DlgListProgramming() {
        setTitle("Lista de Programación");
        ...
        filterButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (text.length() == 0) {
                    ...
                    sorter.setRowFilter(RowFilter.regexFilter("(?i)" + text, 0, 1, 5, 6));
                }
            }
        });
    ...
    private void eventStarter() {
        ...
        tableModel.addColumn("Tipo de Programación");
        tableModel.addColumn("Nombre");
        tableModel.addColumn("Descripción");
        tableModel.addColumn("Fecha de Estreno");
        tableModel.addColumn("Carátula (URL)");
        tableModel.addColumn("Género");
        tableModel.addColumn("Sub-Género");
        tableModel.addColumn("Temporadas");
        tableModel.addColumn("Capítulos");
    ...
    /**
     * Método para actualizar la tabla con un ArrayList de objetos SeriesVO y MoviesVO.
     *
     * @param dataSerie Lista de objetos SeriesVO que se utilizarán para actualizar la tabla.
     * @param dataMovie Lista de objetos MoviesVO que se utilizarán para actualizar la tabla.
     */
    public void updateTable(ArrayList<SeriesVO> dataSerie, ArrayList<MoviesVO> dataMovie) {
        tableModel.setRowCount(0); // Limpia todas las filas
        for (SeriesVO serie : dataSerie) {
            tableModel.addRow(new Object[]{serie.getTypeProgramming(), serie.getProgrammingName(),
                    serie.getDescription(), serie.getReleaseDate(), serie.getCover(), serie.getGender(),
                    serie.getSubGenre(), serie.getNumberSeasons(), serie.getNumberChapter()});
        }

        for (MoviesVO movie : dataMovie) {
            tableModel.addRow(new Object[]{movie.getTypeProgramming(), movie.getProgrammingName(),
                    movie.getDescription(), movie.getReleaseDate(), movie.getCover(), movie.getGender(),
                    movie.getSubGenre()});
        }
